/**
 * Inspect or configure Cloudflare DNS and Email Routing for GitHub Pages.
 * Usage:
...
if (!DNS_TOKEN) {
  throw new Error("CLOUDFLARE_DNS_API_TOKEN is required in .env.local");
}
...
async function zoneInfo() {
  ...
  if (!Array.isArray(matches) || matches.length !== 1) {
    throw new Error(
      `Could not find active Cloudflare zone for ${DOMAIN}. Add the domain to Cloudflare first.`
    );
  }
...
async function dnsRecords() {
  return cf(DNS_TOKEN, `/zones/${ZONE_ID}/dns_records?per_page=500`);
}
...
async function emailStatus() {
  ...
  const [settings, rules] = await Promise.all([
    ...
    cf(EMAIL_TOKEN, `/zones/${ZONE_ID}/email/routing/rules?per_page=200`).catch(
...
async function enableEmailRouting() {
  ...
  if (!settings.enabled || settings.status !== "ready") {
    settings = await cf(EMAIL_TOKEN, `/zones/${ZONE_ID}/email/routing/enable`, {
      method: "POST",
      body: "{}",
    });
...
async function upsertEmailRule(rules, address, destinations) {
  ...
  if (existing) {
    await cf(EMAIL_TOKEN, `/zones/${ZONE_ID}/email/routing/rules/${existing.id}`, {
      method: "PUT",
...
async function configureEmail() {
  ...
  try {
    ...
    throw new Error(
      `The Cloudflare email token needs account-level Email Routing Addresses: Edit permission. ${error.message}`
    );
  }
  const pending = [];
