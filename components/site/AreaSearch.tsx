"use client";
import { useMemo, useState } from "react";
import Link from "next/link";
import { Search, MapPin, Crosshair, Loader2, AlertTriangle } from "lucide-react";
import type { Area } from "@/lib/areas";

const OC_BOUNDS = { minLat: 33.34, maxLat: 33.98, minLng: -118.14, maxLng: -117.40 };

function haversineMi(la1: number, lo1: number, la2: number, lo2: number) {
  const R = 3958.8, toRad = (d: number) => (d * Math.PI) / 180;
  const dLa = toRad(la2 - la1), dLo = toRad(lo2 - lo1);
  const a =
    Math.sin(dLa / 2) ** 2 +
    Math.cos(toRad(la1)) * Math.cos(toRad(la2)) * Math.sin(dLo / 2) ** 2;
  return 2 * R * Math.asin(Math.sqrt(a));
}

type LocState =
  | { phase: "idle" }
  | { phase: "locating" }
  | { phase: "denied" }
  | { phase: "out_of_area"; lat: number; lng: number }
  | { phase: "matched"; lat: number; lng: number };

export function AreaSearch({ areas }: { areas: Area[] }) {
  const [q, setQ] = useState("");
  const [loc, setLoc] = useState<LocState>({ phase: "idle" });

  const nearby = useMemo(() => {
    if (loc.phase !== "matched" && loc.phase !== "out_of_area") return null;
    return areas
      .map((a) => ({ a, dist: haversineMi(loc.lat, loc.lng, a.lat, a.lng) }))
      .sort((x, y) => x.dist - y.dist)
      .slice(0, 8);
  }, [areas, loc]);

  const filtered = useMemo(() => {
    const s = q.trim().toLowerCase();
    if (!s) return areas;
    return areas.filter(
      (a) =>
        a.name.toLowerCase().includes(s) ||
        a.city.toLowerCase().includes(s) ||
        (a.zip && a.zip.some((z) => z.includes(s)))
    );
  }, [q, areas]);

  function detect() {
    if (loc.phase === "locating") return;
    setLoc({ phase: "locating" });
    if (!("geolocation" in navigator)) {
      setLoc({ phase: "denied" });
      return;
    }
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        const { latitude: lat, longitude: lng } = pos.coords;
        const inOC =
          lat >= OC_BOUNDS.minLat &&
          lat <= OC_BOUNDS.maxLat &&
          lng >= OC_BOUNDS.minLng &&
          lng <= OC_BOUNDS.maxLng;
        setLoc({ phase: inOC ? "matched" : "out_of_area", lat, lng });
      },
      () => setLoc({ phase: "denied" }),
      { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }
    );
  }

  const showNearby =
    (loc.phase === "matched" || loc.phase === "out_of_area") && nearby && nearby.length > 0;
  const topMatch = showNearby ? nearby![0] : null;

  return (
    <div>
      <div className="mx-auto max-w-2xl">
        {/* Mobile: prominent stand-alone CTA above the search */}
        <button
          type="button"
          onClick={detect}
          disabled={loc.phase === "locating"}
          className="mb-3 flex w-full items-center justify-center gap-2 rounded-2xl border border-brass-500/60 bg-gradient-to-b from-brass-500 to-brass-600 px-5 py-3 text-sm font-extrabold uppercase tracking-wider text-ink-950 shadow-lg shadow-brass-500/20 transition hover:from-brass-400 hover:to-brass-500 disabled:opacity-60 md:hidden"
        >
          {loc.phase === "locating" ? (
            <>
              <Loader2 className="h-4 w-4 animate-spin" /> Locating you…
            </>
          ) : (
            <>
              <Crosshair className="h-4 w-4" /> Use my location
            </>
          )}
        </button>
        <div className="relative">
          <Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-ink-400" />
          <input
            type="search"
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="Search city, neighborhood, or ZIP…"
            className="h-14 w-full rounded-2xl border border-ink-800 bg-ink-900/70 pl-12 pr-4 text-base outline-none transition-colors focus:border-brass-500 md:pr-44"
          />
          {/* Desktop / tablet: inline pill on the right of the input */}
          <button
            type="button"
            onClick={detect}
            disabled={loc.phase === "locating"}
            className="absolute right-2 top-1/2 hidden h-10 -translate-y-1/2 items-center gap-2 rounded-xl bg-brass-500 px-4 text-xs font-bold uppercase tracking-wider text-ink-950 transition hover:bg-brass-400 disabled:opacity-60 md:inline-flex"
          >
            {loc.phase === "locating" ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" /> Locating…
              </>
            ) : (
              <>
                <Crosshair className="h-4 w-4" /> Use my location
              </>
            )}
          </button>
        </div>
        <p className="mt-2 text-center text-[11px] uppercase tracking-wider text-ink-500">
          Real-time GPS · matches you to the nearest OC service area
        </p>
      </div>

      {loc.phase === "denied" && (
        <div className="mx-auto mt-6 max-w-2xl rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 text-sm">
          <div className="flex items-center gap-2 font-bold text-amber-300">
            <AlertTriangle className="h-4 w-4" /> Location blocked
          </div>
          <p className="mt-1 text-xs text-ink-300">
            Type your city, neighborhood, or ZIP above and we&rsquo;ll find your service area instantly.
          </p>
        </div>
      )}

      {loc.phase === "out_of_area" && topMatch && (
        <div className="mx-auto mt-6 max-w-2xl rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 text-sm">
          <div className="flex items-center gap-2 font-bold text-amber-300">
            <AlertTriangle className="h-4 w-4" /> Outside our Orange County service zone
          </div>
          <p className="mt-1 text-xs text-ink-300">
            Closest OC service area to you is{" "}
            <Link
              href={`/service-areas/${topMatch.a.slug}`}
              className="font-semibold text-brass-300 underline"
            >
              {topMatch.a.name}
            </Link>{" "}
            (~{topMatch.dist.toFixed(0)} mi). Give dispatch a call to confirm coverage.
          </p>
        </div>
      )}

      {loc.phase === "matched" && topMatch && (
        <div className="mx-auto mt-6 max-w-3xl rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-5">
          <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-emerald-300">
            <span className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" />
            Live location locked
          </div>
          <h3 className="mt-2 font-display text-xl font-extrabold md:text-2xl">
            You&rsquo;re closest to{" "}
            <Link
              href={`/service-areas/${topMatch.a.slug}`}
              className="text-brass-gradient underline-offset-4 hover:underline"
            >
              {topMatch.a.name}
            </Link>{" "}
            <span className="text-base font-semibold text-ink-400">
              ({topMatch.dist.toFixed(1)} mi)
            </span>
          </h3>
          <p className="mt-1 text-sm text-ink-300">
            Nearest areas to your real-time GPS pin — tap any to see local dispatch info.
          </p>
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {nearby!.map(({ a, dist }) => (
              <li key={a.slug}>
                <Link
                  href={`/service-areas/${a.slug}`}
                  className="flex items-center justify-between gap-3 rounded-xl border border-ink-800 bg-ink-950/50 px-3 py-2 text-sm transition hover:-translate-y-0.5 hover:border-brass-500/40"
                >
                  <span className="flex items-center gap-2">
                    <MapPin className="h-4 w-4 text-brass-400" />
                    <span className="font-semibold">{a.name}</span>
                  </span>
                  <span className="text-xs font-bold text-emerald-300">
                    {dist.toFixed(1)} mi
                  </span>
                </Link>
              </li>
            ))}
          </ul>
        </div>
      )}

      <p className="mt-8 text-center text-sm text-ink-400">
        Showing {filtered.length} of {areas.length} areas
      </p>
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filtered.map((a) => (
          <Link
            key={a.slug}
            href={`/service-areas/${a.slug}`}
            className="group flex items-center justify-between gap-3 rounded-xl border border-ink-800 bg-ink-900/40 p-3 transition-all hover:-translate-y-0.5 hover:border-brass-500/40"
          >
            <div className="flex items-center gap-3">
              <MapPin className="h-4 w-4 text-brass-400" />
              <div>
                <div className="text-sm font-semibold">{a.name}</div>
                <div className="text-xs capitalize text-ink-400">
                  {a.kind === "city"
                    ? "City"
                    : a.kind === "neighborhood"
                    ? `${a.city} neighborhood`
                    : a.kind === "community"
                    ? "Community"
                    : `ZIP ${a.zip?.[0] ?? ""}`}
                </div>
              </div>
            </div>
          </Link>
        ))}
      </div>
    </div>
  );
}
