...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-ink-800/80 glass">
      ...
      <div
        className={cn(
          "lg:hidden border-t border-ink-800 transition-[max-height] duration-300",
          open
            ? "max-h-[calc(100dvh-4rem)] overflow-y-auto overscroll-contain"
            : "max-h-0 overflow-hidden"
        )}
      >
        <div className="mx-auto flex max-w-7xl flex-col gap-1 px-4 py-3 pb-[max(env(safe-area-inset-bottom),5rem)]">
          {NAV.map((n) => (
            <Link
              key={n.href}
              href={n.href}
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            {BIZ.social.instagram && (
              ...
              >
                <Instagram className="h-3.5 w-3.5" /> @bhairductcleaningmetrodetroit
              </a>
            )}
          </div>
          <div className="mt-3">
